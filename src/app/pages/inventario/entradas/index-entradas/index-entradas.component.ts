...
import { FormEntradasComponent } from '../form-entradas/form-entradas.component';
...
export class IndexEntradasComponent implements OnInit {
  ...
  onLoadData() {
    ...
    this.dataService
      .get<any[]>(
        `EntradaProducto/GetEntradaProductos/${this.customerSelectService.customerId}`
      )
      .subscribe(
        (resp: any) => {
          this.data = resp.body;
  ...
  onDelete(data: any) {
    ...
    Swal.showLoading();
    this.dataService.delete(`EntradaProducto/${data.id}`).subscribe(
      (resp) => {
