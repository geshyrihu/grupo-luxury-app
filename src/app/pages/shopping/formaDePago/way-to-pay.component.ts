...
import { AddoreditWayToPayComponent } from './addoredit-way-to-pay.component';
...
export class WayToPayComponent implements OnInit {
  ...
  onLoadData() {
    ...
    Swal.showLoading();
    this.dataService.get('FormaPago').subscribe(
      (resp: any) => {
        this.data = resp.body;
        Swal.close();
      },
  ...
  onDelete(dto: any) {
    ...
    Swal.showLoading();
    this.dataService.delete(`FormaPago/${dto.id}`).subscribe(
      (resp) => {
  ...
  showModalAddOrEdit(data: any) {
    this.ref = this.dialogService.open(AddoreditWayToPayComponent, {
      data: {
        id: data.id,
      },
      header: data.title,
