...
export class EditOrdenCompraFijosComponent implements OnInit {
  ...
  onLoadData() {
    ...
    Swal.showLoading();
    this.dataService.get<any>(`OrdenCompra/${this.ordenCompraId}`).subscribe(
      (resp: any) => {
  ...
  onRevisadaPorResidente() {
    Swal.fire({
      title:
        '¿Confirmas haber revisado corectamente los datos de esta orden de compra?',
      icon: 'info',
      showDenyButton: true,
  ...
  onSubmitProducto(item: any): void {
    this.dataService.put(`OrdenCompraDetalle/${item.id}`, item).subscribe(
      (resp) => {
        this.onLoadData();
      },
      (err) => {
